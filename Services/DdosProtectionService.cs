...
namespace SecureShopDemo.Services {
    public class DdosProtectionService {
        ...
        public void RecordRequest(string ip, string path, string method) {
            ...
            lock (requestList) {
                ...
                if (requestList.Count > 10) {
                    ...
                    SaveAttackLog(ip, path, method, $"DDoS suspected: {requestList.Count} requests in 10s");
                    requestList.Clear();
                }
            }
        }
        ...
        // ✅ MỚI: Lấy số request hiện tại của IP
        public int GetRequestCount(string ip) {
            if (_requests.TryGetValue(ip, out var list)) {
                lock (list) {
                    var now = DateTime.UtcNow;
                    return list.Count(x => x >= now.AddSeconds(-10));
                }
            }
            return 0;
        }
